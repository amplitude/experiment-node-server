...
describe('Exposure.canonicalize()', () => {
  test('canonicalizes with string user_id and device_id', () => {
    ...
    expect(exposure.canonicalize()).toBe(
  ...
  test('handles non-string user_id and device_id without throwing', () => {
    ...
    const exposure = new Exposure(user, results);
    expect(() => exposure.canonicalize()).not.toThrow();
    expect(exposure.canonicalize()).toBe('12345 67890 flag-key-1 999 ');
  });
});
